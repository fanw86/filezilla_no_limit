...
- `filezilla3-sftp-segmented-download.patch` adds opt-in segmented SFTP downloads
  (engine options "SFTP download segments" / "SFTP segment min size"); the segment
  connections feature lives in `src/engine/sftp/segment_connection.{h,cpp}` and is
  wired into `src/engine/Makefile.am` — new engine source files must stay listed there.
